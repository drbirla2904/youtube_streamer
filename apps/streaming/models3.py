...
class Playlist(models.Model):
    ...
    playlist_id = models.CharField(max_length=255, unique=True)  # YouTube playlist ID
...
class Stream(BaseModel):
    ...
    playlist = models.ForeignKey(Playlist, on_delete=models.SET_NULL, null=True, blank=True, related_name='streams')
    ...
    scheduled_start_time = models.DateTimeField(null=True, blank=True, db_index=True, help_text='When the stream should automatically start')
    started_at = models.DateTimeField(null=True, blank=True)
    stopped_at = models.DateTimeField(null=True, blank=True)
    # In Stream class, add/enhance:
    direct_stream = models.BooleanField(default=False, help_text="Stream directly without download")
    channel_id = models.CharField(max_length=255, null=True, blank=True)  # For channel selection

    # Update clean() to validate:
    def clean(self):
        if self.direct_stream and not self.playlist:
            raise ValidationError("Direct stream requires playlist selection")
        super().clean()
